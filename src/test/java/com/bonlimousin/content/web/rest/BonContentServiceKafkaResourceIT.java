...
import com.bonlimousin.content.config.KafkaProperties;
...
class BonContentServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        BonContentServiceKafkaResource kafkaResource = new BonContentServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bon-content-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bon-content-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
